...
RUN	set -x \
    ...
    && wget --no-check-certificate -c --header "Cookie: oraclelicense=accept-securebackup-cookie" http://download.oracle.com/otn-pub/java/jdk/${VERSION}-b${JAVA_BUILD}/${JAVA_PATH}/jdk-${VERSION}-linux-x64.tar.gz | tar -zcvf - -C /tmp \
    && mkdir -p /usr/lib/jvm \
